...
void menu::draw()
{
	...
	if (menu_open) {
		...
		ImGui::Begin("Lockdown Cheat Austrian distribution");
		...
		ImGui::BeginChild("Seitenhalte", ImVec2(75, 0), true);
		if (ImGui::Selectable("Schauen", selected_tab == 1)) selected_tab = 1;
		if (ImGui::Selectable("Cheats", selected_tab == 2)) selected_tab = 2;
		if (ImGui::Selectable("PUFF´n", selected_tab == 3)) selected_tab = 3;
		if (ImGui::Selectable("Andre kacke", selected_tab == 4)) selected_tab = 4;
		if (ImGui::Selectable("Skill issue", selected_tab == 5)) selected_tab = 5;
		ImGui::Separator();
		ImGui::Text("Einstellungen");
		ImHotkey("##MenuHotkey", &menu_hotkey);
		if (ImGui::Button("Speicher")) {
			SaveConfig();
		}
		if (ImGui::Button("WiederLaden")) {
			LoadConfig();
		}
		ImGui::Separator;
				...
				if (ImGui::BeginTabBar("ESPTabBar")) {
					...
					if (ImGui::BeginTabItem("Player")) {
						...
						if (ImGui::CollapsingHeader("Details##PlayerESPDetails", ImGuiTreeNodeFlags_DefaultOpen)) {
							...
							ImGui::Checkbox("Ghostbusters##PlayerRadar", &ghost_radar);
							ImGui::Separator();
					...
					if (ImGui::BeginTabItem("World")) {
						...
						ImGui::SameLine(); MenuTooltip("NUr wenns du der host bist");
		...
		// PLAYER
			...
			if (ImGui::CollapsingHeader("Toggle Hacks##PlayerToggleHacks", ImGuiTreeNodeFlags_DefaultOpen)) {
				ImGui::Checkbox("REN OADA", &speedhack);
				ImGui::SameLine();
				ImHotkey("##SpeedhackHotkey", &speedhack_hotkey);

				ImGui::Checkbox("Geh varueckt", &god_mode);
				ImGui::SameLine();
				ImHotkey("##GodmodeHotkey", &god_mode_hotkey);

				ImGui::Checkbox("KRANKENVERSICHERUNG", &fast_hp_recovery);

				ImGui::Checkbox("Energydrinkovich", &infinite_stamina);
				ImGui::SameLine();
				ImHotkey("##InfstamHotkey", &infinite_stamina_hotkey);

				ImGui::Checkbox("Trink ein energy drink lieber als diese setting", &fast_stam_recovery);
				...
				*/
				...
				ImGui::Checkbox("Fliegen", &fly_mode); ImGui::SameLine(); MenuTooltip("Sei amoai tot");
						...
						if (name_str == "Austria") {
							if (ImGui::Button("Admin")) {
								...
								if (obj) {
									...
									std::cout << "[MainGI] Austria" << std::endl;
									admin_toggle = true;
								}
							}
						}
		...
		// WEAPON
			...
			if (ImGui::CollapsingHeader("MELEE", ImGuiTreeNodeFlags_DefaultOpen)) {
				ImGui::Checkbox("Ich bin kack in melee", &fast_melee);
				ImGui::SameLine();
			...
			if (ImGui::CollapsingHeader("GUN", ImGuiTreeNodeFlags_DefaultOpen)) {
				...
				ImGui::Checkbox("Ka Rueckstoß", &no_recoil);
				ImGui::SameLine();
				ImHotkey("##NoRecoilHotkey", &no_recoil_hotkey);

				ImGui::Checkbox("Oneshot", &max_damage);
				ImGui::SameLine();
				ImHotkey("##MaxDamageHotkey", &max_damage_hotkey);

				ImGui::Checkbox("ImmerMuni", &infinite_ammo);
			...
			if (rapid_fire || no_recoil || infinite_ammo) {
				if (ImGui::CollapsingHeader("Gun Details##GunDetails", ImGuiTreeNodeFlags_DefaultOpen)) {
					...
					if (infinite_ammo) {
						ImGui::SetNextItemWidth(ImGui::GetContentRegionAvail().x);
						ImGui::SliderInt("##ammo", &ammo_count, 0, 100, "Ammo: %d");
					}
				}
			}
			if (impact_change) {
			...
			if (ImGui::CollapsingHeader("INVENTORY", ImGuiTreeNodeFlags_DefaultOpen)) {
				if (hand_item) {
					...
					if (hand_item_name == "GAZ BOTTLE") {
						...
						ImGui::SetNextItemWidth(ImGui::GetContentRegionAvail().x);
						if (ImGui::SliderInt("##HandAmmo", &ammo, 0, 500, "Ammo: %d%")) {
